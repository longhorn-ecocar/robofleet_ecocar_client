...
template <>
flatbuffers::uoffset_t encode(
    FBB& fbb, const std_msgs::MultiArrayDimension& msg,
    const MetadataOffset& metadata) {
  // Here you can use the autogenerated function for MultiArrayDimensionDirect.
  return fb::std_msgs::CreateMultiArrayDimensionDirect(
      fbb, metadata,
      msg.label.c_str(),  // assuming label is a std::string
      msg.size,
      msg.stride).o;
}
...
template <>
flatbuffers::uoffset_t encode(
    FBB& fbb, const std_msgs::MultiArrayLayout& msg,
    const MetadataOffset& metadata) {
  // Encode the 'dim' vector (each element is a std_msgs::MultiArrayDimension)
  auto dim_offset = encode_vector<std_msgs::MultiArrayDimension>(fbb, 0, msg.dim);
  // Use the autogenerated FlatBuffers function (non-Direct version) to create the layout.
  return fb::std_msgs::CreateMultiArrayLayout(
    fbb, metadata, dim_offset, msg.data_offset).o;
}
...
flatbuffers::uoffset_t encode(
    ...
    const MetadataOffset& metadata) {
  auto layout_offset = encode(fbb, msg.layout, 0);
  return fb::std_msgs::CreateByteMultiArrayDirect(
    fbb, metadata, layout_offset, &msg.data).o;
}
